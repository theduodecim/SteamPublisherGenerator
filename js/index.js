...
$('#start').on('click', function () {
  ...
  for (i = 0; i < 20; i++) {
      ...
      xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          ...
          $('<ul>')
            ...
            .appendTo('#Pos');
          $('<ol>').appendTo(
            $('<ul>').text(appitemname.substring(0, 15))
            .appendTo('#GameName'));
    ...
    setTimeout(function () {
      ...
      xmlhttp.open("GET", "http://store.steampowered.com/api/appdetails?appids=" + appitemid, false);
      var dirtest = 'http://store.steampowered.com/api/appdetails?appids=' + appitemid;
      xmlhttp.send();
    }, 10)
    setTimeout(function () {
      var xmlhttp = new XMLHttpRequest();
      xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          ...
          $('<ol>').appendTo(
            $('<li>').text(website)
            .appendTo($('<a>').attr('href', website).appendTo('#link')));
          //console.log(myObj.applist.apps[4534]);
        } else {
